						   ...
						   uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count)
{
	...
	if (Creature* movingCreature = thing->getCreature()) {
		...
		if (Position::areInRange<1, 1, 0>(movingCreature->getPosition(), player->getPosition())) {
			SchedulerTask* task = createSchedulerTask(600,
					 ...
					 const std::string& receiver, const std::string& text)
{
    ...
    for (auto it : wordsList) {
        if (newText.find(asLowerCaseString(it)) != std::string::npos) {
            std::ostringstream ss;
            ss << "Anti-word system EmperaOT. you will be banned if you continue.";
            player->sendTextMessage(MESSAGE_STATUS_SMALL, ss.str());
            return;
        }
    }
...
void Game::playerSpeakToNpc(Player* player, const std::string& text)
{
	...
	if (Condition* conditionnpc = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_EXHAUST, 500, 0, false, 1)) {
