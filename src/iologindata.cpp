...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	query.str(std::string());
	query << "SELECT `pid`, `sid`, `itemtype`, `count`, `attributes` FROM `player_depotitems` WHERE `player_id` = " << player->getGUID() << " ORDER BY `sid` DESC LIMIT 3000";
	if ((result = db.storeQuery(query.str()))) {
		loadItems(itemMap, result);
	...
	query.str(std::string());
	query << "SELECT `pid`, `sid`, `itemtype`, `count`, `attributes` FROM `player_rewards` WHERE `player_id` = " << player->getGUID() << " ORDER BY `sid` DESC LIMIT 800";
    if ((result = db.storeQuery(query.str()))) {
		loadItems(itemMap, result);
	...
	query.str(std::string());
	query << "SELECT `pid`, `sid`, `itemtype`, `count`, `attributes` FROM `player_inboxitems` WHERE `player_id` = " << player->getGUID() << " ORDER BY `sid` DESC LIMIT 800";
	if ((result = db.storeQuery(query.str()))) {
		loadItems(itemMap, result);
