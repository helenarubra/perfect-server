...
bool ConfigManager::load()
{
	...
	integer[MAX_ITEM_TRANSFER_DEPOT] = getGlobalNumber(L, "maxTransferHouseItems", 1000);
	...
	integer[ACTIONS_DELAY_INTERVAL] = getGlobalNumber(L, "timeBetweenActions", 800);
	...
	integer[PROTECTION_LEVEL] = getGlobalNumber(L, "protectionLevel", 20);
	...
	integer[FRAG_TIME] = getGlobalNumber(L, "timeToDecreaseFrags", 15 * 60 * 60);
	...
	integer[RED_SKULL_DURATION] = getGlobalNumber(L, "redSkullDuration", 2);
	integer[BLACK_SKULL_DURATION] = getGlobalNumber(L, "blackSkullDuration", 3);
